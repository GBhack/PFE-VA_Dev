...
def set_frequency(channelFrequency):
    """
        channelFrequency in Hertz
        Set the frequency of both PWM channels
    """
    PWM.set_frequency(MOTOR_LEFT["PWM"], channelFrequency)
    PWM.set_frequency(MOTOR_RIGHT["PWM"], channelFrequency)

def set_period(channelPeriod):
    """
        channelPeriod in seconds
        Allow to use seconds instead of Hertz
    """
    set_frequency(1/channelPeriod)

 

SOCKETS = RB.sockets

CONNEXION = SOCKETS.tcp.Server.Server(RB.constants.ports.FL["pwm"])
...
CONNEXION.listen_to_clients(set_settings_cb, ARGUMENTS)
